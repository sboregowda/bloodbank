...
if (loggedIn()) {
	$sOutput .= '<h2>Welcome!</h2>
		Hello, ' . $_SESSION['username'] .'<br/><br />';
			...
			//ASK Blood Request
			$sql = "SELECT * FROM DonateBlood_Request JOIN users ON DonateBlood_Request.username=users.username WHERE users.bloodtype = '" . $_SESSION['bloodtype'] . "' and users.city='".$_SESSION['city']."' and users.username != '".$_SESSION['username']."'";
			
			//echo "<br/>".$sql;
			
			// Note the use of trigger_error instead of or die.
			$query = mysql_query($sql) or trigger_error("Query Failed: " . mysql_error());
			$response="<h5>List of donors of your bloodtype ". $_SESSION['bloodtype']." and your city ". $_SESSION['city']."</h5>";
				//
