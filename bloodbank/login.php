		...
		case 'login':
			if (isset($_POST['username']) && isset($_POST['password'])) {
				...
				if (!validateUser($_POST['username'], $_POST['password'])) {
					...
					$_SESSION['error'] = "<font color='red'>Bad username or password supplied.</font>";
					unset($_GET['action']);
				}
			}else {
...
if (loggedIn()) {
	$sOutput .= '<h2>Welcome to Student Issue Discussion Forum</h2><br />';
	$sOutput .= '<h1>Logged In!</h1><br /><br />
		Hello, ' . $_SESSION["username"] .'('.$_SESSION['usertype'].')'. ' <br /><br />
